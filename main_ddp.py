...
def main(config: SketchModelConfig):
    ...
    if config.evaluate:
        test_stats = evaluate(config, model, criterion, data_loader_val,
                              device)
        test_stats_weight = evaluate(config, model, criterion, data_loader_val,
                                     device, "weighted")
        ...
        if utils.is_main_process():
            ...
            print(test_stats_weight[1])
            with open('./log.txt', 'a') as f:
                f.write(json.dumps(test_stats[1]) + "\n")
                f.write(json.dumps(test_stats_weight[1]) + "\n")
        return
...
def train_one_epoch(
    ...
    for (batch_img, batch_name, batch_bbox, batch_color,
        ...
        batch_img: NestedTensor = batch_img.to(device)  # [8, 176, 3, 64, 64]
        batch_name: NestedTensor = batch_name.to(device)  # [8, 176, 32]
        batch_bbox: NestedTensor = batch_bbox.to(device)  # [8, 176, 4]
        batch_color: NestedTensor = batch_color.to(device)  # [8, 176, 4]
        batch_class: NestedTensor = batch_class.to(device)  # [8, 176]
        outputs = model(
            flatten_input_for_model(batch_img, batch_name, batch_bbox,
                                    batch_color,
                                    batch_class))  # [8, 176, 4] SoftMax 输出
